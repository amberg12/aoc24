...
numslist_can_be_target(
	...
	sprintf(catbuf, "%lld%lld", root, nums[i]);
	long long int cat = parsenum(catbuf);
	...
	bool cat_ok = numslist_can_be_target(nums, nelems, i+1, cat, target);

	return mult_ok || add_ok || cat_ok;
}
